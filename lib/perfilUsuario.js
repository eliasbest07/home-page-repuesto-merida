...
// Información oficial del usuario — FUENTE DE VERDAD: /users
//   /users/{uidGoogle}  →  usuarios viejos de la app Android
//   /users/{telefono}   →  usuarios web (registro/perfil escriben aquí)
...
// la info hay que emparejar ese número contra users.whatsapp normalizando
// ambos a la misma forma canónica.
//
// `rifas_usuarios` ya NO almacena el perfil: es exclusivo del flujo de rifas.
// Se lee aquí solo como fallback legacy para datos antiguos no migrados.
 ...
 */
export async function buscarUsuarioOficial(telefono, { rtdb = null } = {}) {
...
export function construirPerfil({ telefono, rifas, oficial }) {
  ...
  // /users es primario; rifas_usuarios queda como fallback legacy.
  const lat = oficial?.lat ?? ubic?.lat ?? rifas?.lat ?? null
  const lng = oficial?.lng ?? ubic?.lng ?? rifas?.lng ?? null
  const nombre = String(oficial?.nombre || oficial?.google_nombre || rifas?.nombre || '').trim()
  const foto_url = oficial?.foto || oficial?.foto_url || rifas?.foto_url || null
  const cedula = String(oficial?.cedula || rifas?.cedula || '').trim()
  ...
  const perfil = {
    ...
    nombre,
    cedula,
    cedula_estado: oficial?.cedula_estado ?? rifas?.cedula_estado ?? '',
    cedula_actualizada_en: oficial?.cedula_actualizada_en ?? rifas?.cedula_actualizada_en ?? null,
    foto_url,
    lat,
    lng,
    ...
    ...(oficial && {
      ...
      modelo: oficial.modelo ?? oficial.vehiculo_modelo ?? null,
      anio: oficial.anio ?? oficial.vehiculo_anio ?? null,
      tipo_vehiculo: oficial.tipo_vehiculo ?? oficial.vehiculo_tipo ?? null,
      ubicacion_texto: oficial.ubicacion_texto ?? null,
      tipovender: oficial.tipovender ?? null,
    }),
    // Datos del comercio (los escribe /api/usuario/comercio en rifas_usuarios).
    comercio_foto_url: oficial?.comercio_foto_url ?? rifas?.comercio_foto_url ?? '',
    comercio_direccion: oficial?.comercio_direccion ?? rifas?.comercio_direccion ?? '',
    comercio_lat: oficial?.comercio_lat ?? rifas?.comercio_lat ?? null,
    comercio_lng: oficial?.comercio_lng ?? rifas?.comercio_lng ?? null,
    vender: oficial?.vender ?? rifas?.vender ?? false,
    ...((oficial?.creado_en ?? rifas?.creado_en) ? { creado_en: oficial?.creado_en ?? rifas?.creado_en } : {}),
  }
 ...
 */
export async function resolverPerfil({ telefono, key, realtimeUid = '' }) {
  const rtdb = getAdminRealtimeDb()
  const [rifasSnap, exactSnap] = await Promise.all([
    rtdb.ref(`rifas_usuarios/${key}`).get(),
    realtimeUid ? rtdb.ref(`users/${realtimeUid}`).get() : Promise.resolve(null),
  ])
  const exact = exactSnap?.exists()
    ? { uid: realtimeUid, ...(exactSnap.val() || {}) }
    : null
  const oficial = exact || await buscarUsuarioOficial(telefono || key, { rtdb })
