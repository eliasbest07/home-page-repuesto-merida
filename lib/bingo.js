...
/**
 * Genera un identificador de cartón de 6 dígitos (ej: "126749").
 */
export function generateCartonId() {
  return String(Math.floor(100000 + Math.random() * 900000));
}

/**
 * Devuelve los cartones de un jugador como lista ordenada por creación.
 * Soporta el formato nuevo `player.cartones = { id: { filas, creadoEn } }`
 * y el legado `player.carton` (un solo cartón sin id).
 */
export function playerCartones(player) {
  if (!player) return [];
  if (player.cartones && typeof player.cartones === 'object') {
    return Object.entries(player.cartones)
      .map(([id, data]) => ({
        id,
        carton: Array.isArray(data) ? data : data?.filas || [],
        creadoEn: data?.creadoEn || 0,
      }))
      .sort((a, b) => a.creadoEn - b.creadoEn || a.id.localeCompare(b.id));
  }
  if (player.carton) {
    return [{ id: 'unico', carton: player.carton, creadoEn: player.joinedAt || 0 }];
  }
  return [];
}
