...
function getSecret() {
  const secret =
    ...
    || (process.env.NODE_ENV !== 'production' ? 'dev-rifa-secret-CHANGE-IN-PRODUCTION' : '')
  if (secret.length < 32) {
    throw new Error('RIFA_JWT_SECRET debe tener al menos 32 caracteres.')
  }
  return secret
}
...
export function signRifaToken(payload, ttlMs = TTL_MS) {
  ...
  const sig = b64url(crypto.createHmac('sha256', getSecret()).update(`${h}.${p}`).digest())
...
export function verifyRifaToken(token) {
  ...
  let expected
  try {
    expected = b64url(crypto.createHmac('sha256', getSecret()).update(`${h}.${p}`).digest())
  } catch {
    return null
  }
